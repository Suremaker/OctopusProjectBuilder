...
using IResource = Octopus.Client.Extensibility.IResource;
	...
	public class ModelUploader
	{
		...
		private TResource Upsert<TRepository, TResource>(TRepository repository, TResource resource) where TResource : IResource, INamedResource where TRepository : ICreate<TResource>, IModify<TResource>
		{
			var result = string.IsNullOrWhiteSpace(resource.Id)
				? repository.Create(Adjust(resource))
				: repository.Modify(Adjust(resource));
		...
		{
			var result = repository.Modify(Adjust(resource));

			Logger.Debug($"Updated {parentName} -> {typeof(TResource).Name}: {resource.Id}");
			return result;
		}

		private TResource Adjust<TResource>(TResource resource) where TResource : IResource
		{
			var process = resource as DeploymentProcessResource;
			if (process == null)
				return resource;
			foreach (var step in process.Steps)
				foreach (var action in step.Actions)
					if (action.Properties.Keys.Contains("Octopus.Action.IISWebSite.DeploymentType"))
						action.Properties.Remove("Octopus.Action.IISWebSite.DeploymentType");
			return resource;
		}
